...
<div class="box-dashboard-wrapper widget">
	<div class="inner-list">
		<h3 class="widget-title"><?php echo esc_html__('Job Alerts','workup') ?></h3>
				...
				<table class="job-table">
					...
					<?php foreach ($alerts->posts as $alert_id) {
						...
						<tr <?php post_class('job-alert-wrapper'); ?>>
							...
							<td>
								<a href="javascript:void(0)" class="btn-remove-job-alert btn-action-icon deleted" data-alert_id="<?php echo esc_attr($alert_id); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-job-board-remove-job-alert-nonce' )); ?>"><i class="ti-close"></i></a>
							</td>
						</tr>
		...
		<?php } else { ?>
			<div class="not-found"><?php esc_html_e('No job alert found.', 'workup'); ?></div>
		<?php } ?>
		</div>
</div>